...
const RESOLVE_TIMEOUT_MS = Number(process.env.RESOLVE_TIMEOUT_MS || 70000);
const VOICE = process.env.JEB_VOICE || 'cedar';
const TTS_MODEL = process.env.TTS_MODEL || 'gpt-4o-mini-tts';
const INDEX = path.join(__dirname, 'index.html');
const BUILD = 'field-play-1.14-environment-sovereignty';
...
async function speak(req, res) {
  if (!OPENAI_API_KEY) return sendJson(req,res,503,{ok:false,error:'OPENAI_API_KEY not configured'});
  try {
    const p=await readJson(req);
    const text=String(p.text||'').trim().slice(0,4096);
    if(!text) return sendJson(req,res,400,{ok:false,error:'text required'});
    const tone={urgent:'urgent but controlled',wary:'low and wary',decisive:'decisive and grounded',warm:'warm with dry wit',neutral:'natural and grounded'}[p.state]||'natural and grounded';
    const response=await fetch('https://api.openai.com/v1/audio/speech',{method:'POST',headers:{Authorization:`Bearer ${OPENAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:TTS_MODEL,voice:VOICE,input:text,instructions:`Adult male frontier storyteller, ${tone}, lived-in and conversational, untheatrical. You are Jeb Highsmith in Round Rock, Texas, 1878.`,response_format:'mp3'})});
    if(!response.ok) return sendJson(req,res,502,{ok:false,error:'tts_failed',status:response.status,detail:(await response.text()).slice(0,400)});
    const d=Buffer.from(await response.arrayBuffer());
    res.writeHead(200,{'Content-Type':'audio/mpeg','Content-Length':d.length,'Cache-Control':'no-store'});res.end(d);
  } catch(e) { return sendJson(req,res,500,{ok:false,error:'tts_exception',detail:String(e.message||e)}); }
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && url.pathname === '/health') {
    return sendJson(req,res,200,{ok:true,service:'round-rock-1878-frontend',build:BUILD,voice:VOICE,tts_model:TTS_MODEL,api_key_configured:Boolean(OPENAI_API_KEY),resolver:'local'});
  }
  if (req.method === 'GET' && url.pathname === '/resolver/health') {
    return sendJson(req,res,200,{ok:true,service:'contentstream-six-questions-only',model:OPENAI_MODEL,model_key_configured:Boolean(OPENAI_API_KEY),endpoints:['/resolver/resolve','/resolver/field-event','/resolver/health']});
  }
  if (req.method === 'POST' && url.pathname === '/speak') return speak(req,res);

  if (req.method === 'POST' && (url.pathname === '/resolve' || url.pathname === '/resolver/resolve')) {
  ...
  if (req.method === 'POST' && (url.pathname === '/field-event' || url.pathname === '/resolver/field-event')) {
    try {
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`Round Rock 1.14 environment-sovereignty resolver listening on :${PORT}`);
